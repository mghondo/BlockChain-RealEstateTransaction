...
// Declares the MockAUSDC contract inheriting from ERC20.
contract MockAUSDC is ERC20 {
  ...
  constructor(address _usdc) ERC20('Mock aUSDC', 'aUSDC') {
    // Constructor taking USDC address and initializing ERC20 with name and symbol.
  ...
  function burn(address from, uint256 amount) external {
    // External function for burning tokens with yield adjustment.
    ...
    uint256 baseToBurn = (amount == yieldedBalance)
      ? baseBalance
      : (amount * baseBalance) / yieldedBalance; // Computes base amount to burn proportionally.
  ...
  function transferUSDC(address to, uint256 amount) external {
    // External function to transfer USDC, simulating yield mint if needed.
    ...
    if (currentUsdcBalance < amount) {
      // Checks if balance is insufficient.
  ...
  function balanceOf(address account) public view override returns (uint256) {
    // Overrides balanceOf to add simulated yield.
    ...
    return
      baseBalance + (baseBalance * yieldRatePerSecond * secondsElapsed) / 1e18; // Adds calculated yield to base.
  }
  // Overrides the decimals function to return 6.  // Comment on the decimals override.
  function decimals() public view virtual override returns (uint8) {
    // Overrides decimals to match USDC's 6 decimals.
