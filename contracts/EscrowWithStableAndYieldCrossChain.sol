...
contract EscrowWithStableAndYieldCrossChain is
  ReentrancyGuard,
  NonblockingLzApp
{
  struct Config {
  ...
  function _nonblockingLzReceive(
    ...
    (address buyer, uint256 amount, uint256 shares) = abi.decode(
      _payload,
  ...
  function finalizeSale()
    external
    nonReentrant
    timelockReady(keccak256(abi.encodePacked('FINALIZE_SALE')))
  {
  ...
  function cancelSale()
    external
    nonReentrant
    timelockReady(keccak256(abi.encodePacked('CANCEL_SALE')))
  {
    if (currentPhase.id == 4 || currentPhase.id == 5)
      revert InvalidPhase(3, currentPhase.id);
    ...
    if (isSellerBackingOut || !inspectionPassed) {
      ...
      for (uint i = 0; i < buyers.length; i++) {
        ...
        uint256 buyerYield = totalDeposited == 0
          ? 0
          : (yield * buyerPrincipal) / totalDeposited;
      ...
      if (lenderDeposited > 0) {
        uint256 lenderYield = totalDeposited == 0
          ? 0
          : (yield * lenderDeposited) / totalDeposited;
  ...
  ) external view returns (bool isPending, uint256 executeAfter) {
